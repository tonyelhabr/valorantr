...
cleanly_unnest_players <- function(players) {
  players |> 
    hoist(
      data,
      "ign",
...
nrow(presaved_players)
presaved_players |> 
  filter(player_id %in% optic_liquid_player_names$player_id) |> 
  cleanly_unnest_players()
```
...
str(optic_liquid_match_details, max.level = 1)
```
